    ...
    d <- calcNormFactors(d, refColumn=nrow(targets)-1, method=c(Args[1]))
    ...
    d <- estimateTagwiseDisp(d, prior.n=5, trend=TRUE, prop.used=0.2)
...
#changed loess normalize procedure to average TMM normalized counts instead of fold change data.
    avgreads <- rowSums(d$pseudo.alt)/nrow(targets)-1

#    fcandlocations <- merge (toptags_tgw.out$table, genelocations, by=0)
    fcandlocations <- merge (avgreads, genelocations, by=0)
...
#    y.ratio <- 2^y.predict
    y.ratio <- y.predict/mean(y.predict)
    ...
    plotMDS.dge(d, main='MDS Plot non normalized on measured and expected counts', xlim=c(-3,3), ylim=c(-3,3))
    dev.off()
    d <- calcNormFactors(d, refColumn=nrow(d$samples), method=c(Args[1]))
    d <- estimateCommonDisp(d)
    ...
    loess_essentialgenes_minomics <-toptags_tgw.out$table[toptags_tgw.out$table[,1]>-30, ]
    colnames(loess_essentialgenes_minomics)[2] <- "Essential genes Log2 fold change position bias corrected"
    colnames(loess_essentialgenes_minomics)[4] <- "Essential genes FDR corrected P position bias corrected"
