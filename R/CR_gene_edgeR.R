...
biplot(p3, cex=0.5:1.8, col=c(40,1), main='PCA Plot non normalized on genes')
...
design <- model.matrix(~ library + group, data = targets)

d <- estimateGLMTrendedDisp(d, design, as.numeric(Args[4]))
if (Args[3] == "tagwise") d <- estimateGLMTagwiseDisp(d, design, method="trend", as.numeric(Args[4]))

p3 <- prcomp(d$pseudo.alt)
png('gene_PCA_normalized.png', width=1280, height=1024)
biplot(p3, cex=0.5:1.8, col=c(40,1), main='PCA Plot normalized on genes')
#plotMDS.dge(d, main='MDS Plot normalized on genes', xlim=c(-3,3), ylim=c(-3,3))
dev.off()

if (Args[3] == "common")  glmfit.tgw <- glmFit(d, design, dispersion = d$trended.dispersion)
if (Args[3] == "tagwise") glmfit.tgw <- glmFit(d, design, dispersion = d$tagwise.dispersion)

de.tagwise <- glmLRT(d, glmfit.tgw)
